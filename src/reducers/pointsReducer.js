...
const getPoints = (cat) => {
  ...
  if (cat.recycling.status != true) {
    for (var prop in cat.recycling) {
      if (cat.recycling[prop] == true) {
        if (prop == "clothes") {
          let current = recyclingPoints.features.filter(
            (element) =>
              element.properties.recycling_type == prop &&
              element.properties.object_type == "container"
          );
          array = array.concat(current);
        } else {
          if (prop == "organization") {
            let current = recyclingPoints.features.filter(
              (element) => element.properties.object_type == prop
            );
            array = array.concat(current);
          } else {
            let current = recyclingPoints.features.filter(
              (element) => element.properties.recycling_type == prop
            );
            array = array.concat(current);
          }
        }
  ...
  if (cat.shop.status != true) {
    ...
    let current = recyclingPoints.features.filter(
      (element) =>
        element.properties.object_type == "repairShop" ||
        element.properties.object_type == "secondHandStore" ||
  ...
  if (cat.yard.status == true) {
    let current = recyclingPoints.features.filter(
      (element) => element.properties.object_type == "recyclingYard"
    );
    array = array.concat(current);
  }
  return array;
